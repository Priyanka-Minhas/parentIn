...
public class ProgressBarCircularIndeterminate extends CustomView {
    ...
    private void setAttributes(@NonNull AttributeSet attrs) {
        ...
        setMinimumHeight(Utils.Companion.dpToPx(32, getResources()));
        setMinimumWidth(Utils.Companion.dpToPx(32, getResources()));
        ...
        setMinimumHeight(Utils.Companion.dpToPx(3, getResources()));
    ...
    private void drawFirstAnimation(@NonNull Canvas canvas) {
        if (radius1 < getWidth() / 2) {
            ...
            if (cont >= 50) {
                ...
                radius2 = (radius2 >= getWidth() / 2 - Utils.Companion.dpToPx(4, getResources())) ? (float) getWidth() / 2 - Utils.Companion.dpToPx(4, getResources()) : radius2 + 1;
            }
            temp.drawCircle(getWidth() / 2, getHeight() / 2, radius2, transparentPaint);
            canvas.drawBitmap(bitmap, 0, 0, new Paint());
            if (radius2 >= getWidth() / 2 - Utils.Companion.dpToPx(4, getResources()))
                cont++;
...
//		temp.drawARGB(0, 0, 0, 255);
        ...
        transparentPaint.setColor(getResources().getColor(android.R.color.transparent));
        transparentPaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.CLEAR));
        temp.drawCircle(getWidth() / 2, getHeight() / 2, (getWidth() / 2) - Utils.Companion.dpToPx(4, getResources()), transparentPaint);
